...
function initialiseExp() {
    ...
    k = 648868942 * (2.7182818284590452353602874713527 ** (-45574 / (8.314 * (setTemp + 273))));
...
function setLPH(sideLPH) {
    ...
    if (isNaN(document.getElementById("condVal").innerHTML)) {
        document.getElementById("condVal").innerHTML = (0.00).toFixed(2);
    }
    document.getElementById("addReading").onclick = function() { addReadingsToTable(eaLPH, shLPH, Number(actCond.toFixed(2))) };
    displayMsg();
}
...
function calcConductivity() {
    ...
    Xa = ((1 + molRatio + (1 / (k * tau * CaO))) - Math.pow((Math.pow(1 + molRatio + (1 / (k * tau * CaO)), 2) - (4 * molRatio)), 0.5)) / 2;
